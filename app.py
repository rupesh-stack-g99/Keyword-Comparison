...
st.set_page_config(page_title="SE Ranking Comparison", layout="wide")
...
def extract_month_from_pdf(pdf_file):
    ...
    try:
        with pdfplumber.open(pdf_file) as pdf:
            ...
            # Match date patterns like APR-16 2026 or MAY-15 2026
            months = re.findall(r'(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)', first_page_text, re.IGNORECASE)
            if months:
                # Format cleanly e.g., 'Apr' or 'May'
...
def generate_pdf_report(dataframe, engine_name, label_m1, label_m2):
    ...
    story.append(Paragraph(f"Period: {label_m1} vs {label_m2}", styles['SubTitle']))
...
if file_m1 and file_m2:
    ...
    if df_m1 is not None and df_m2 is not None:
        ...
        # Dynamic Subsets using custom column names
